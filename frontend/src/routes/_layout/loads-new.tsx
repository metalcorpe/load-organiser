...
export const Route = createFileRoute("/_layout/loads-new")({
  component: LoadsPage,
})
    ...
    >
      <Flex justify="space-between" align="start">
        <VStack align="start" gap={2} flex={1}>
          <HStack gap={3}>
            <Text fontWeight="semibold">Load #{load.id.slice(-8)}</Text>
            <Text
              fontSize="xs"
              px={2}
              py={1}
              borderRadius="md"
              bg={`${getStatusColor(load.status || "planning")}`}
              color="white"
              fontWeight="semibold"
            >
              {(load.status || "PLANNING").toUpperCase()}
            </Text>
            <Text fontSize="sm" color="gray.600">
